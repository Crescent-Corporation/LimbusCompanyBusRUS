...
namespace LimbusLocalizeRUS
{
    ...
    {
        public static ConfigEntry<bool> AutoUpdate = LCB_CresCorpMod.CresCorp_Settings.Bind("CresCorp Settings", "AutoUpdate", false, "Автоматическое обновление локализации игры ( true | false )");
        public static void StartAutoUpdate()
        {
            if (AutoUpdate.Value)
            {
                LCB_CresCorpMod.LogWarning("Check Mod Update");
        ...
        static void CheckModUpdate()
        {
            ...
            if (www.result != UnityWebRequest.Result.Success)
                LCB_CresCorpMod.LogWarning("Ошибка со связью с GitHub!" + www.error);
            else
            {
                ...
                if (Version.Parse(LCB_CresCorpMod.VERSION) < Version.Parse(latestReleaseTag.Remove(0, 1)))
                {
                    string updatelog = (latest2ReleaseTag == "v" + LCB_CresCorpMod.VERSION ? "LimbusLocalizeRUS_BIE.7z" : "LimbusLocalizeRUS_BIE.zip") + latestReleaseTag;
                    ...
                    string filename = LCB_CresCorpMod.GamePath + "/" + dirs[^1];
                    if (!File.Exists(filename))
                        DownloadFileAsync(download, filename).GetAwaiter().GetResult();
                    UpdateCall = UpdateDel;
                }
                LCB_CresCorpMod.LogWarning("Check Cyrillic Font Asset Update");
        ...
        static void CheckCyrillicFontAssetUpdate()
        {
            ...
            string FilePath = LCB_CresCorpMod.ModPath + "/tmpcyrillicfonts";
        ...
        {
            LCB_CresCorpMod.OpenGamePath();
            Application.Quit();
        }
        static async Task DownloadFileAsync(string url, string filePath)
        {
            LCB_CresCorpMod.LogWarning("Download " + url + " To " + filePath);
        ...
        public static void CheckReadmeUpdate()
        {
            ...
            www.timeout = 1;
            www.SendWebRequest();
            string FilePath = LCB_CresCorpMod.ModPath + "/Localize/Readme/Readme.json";
