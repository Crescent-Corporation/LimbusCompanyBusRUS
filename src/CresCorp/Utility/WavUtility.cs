...
public class WavUtility
{
    ...
    public static float[] ToAudioData(string filePath)
    {
        if (string.IsNullOrEmpty(filePath)) return null;

        byte[] fileBytes = File.ReadAllBytes(filePath);

        return ToAudioData(fileBytes);
    }
    ...
    public static AudioClip ToAudioClip(byte[] fileBytes, int offsetSamples = 0, string name = "wav")
    {
        ...
        switch (bitDepth)
        {
            ...
            case 32:
                ...
                break;
            default:
                LCB_CresCorpMod.LogInfo(bitDepth + " bit depth is not supported.");
                return null;
        }
    ...
    public static byte[] FromAudioClip(AudioClip audioClip, out string filepath, bool saveAsFile = true, string dirname = "recordings")
    {
        ...
        if (saveAsFile)
        {
            ...
            //LCB_CresCorpMod.LogInfo ("Auto-saved .wav file: " + filepath);
        }
        else
        {
            filepath = null;
